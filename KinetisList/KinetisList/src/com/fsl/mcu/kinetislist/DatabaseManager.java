...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    static {
        ...
        // Maps "Name" to "Name"
        sHeadersProjectionMap.put(DBdefine.Header.COLUMN_NAME_NAME, DBdefine.Header.COLUMN_NAME_NAME);
        // Maps "Title" to "Title"
        sHeadersProjectionMap.put(DBdefine.Header.COLUMN_NAME_TITLE, DBdefine.Header.COLUMN_NAME_TITLE);
        // Maps "type" to "type"
        sHeadersProjectionMap.put(DBdefine.Header.COLUMN_NAME_TYPE, DBdefine.Header.COLUMN_NAME_TYPE);
        // Maps "width" to "width"
        sHeadersProjectionMap.put(DBdefine.Header.COLUMN_NAME_WIDTH, DBdefine.Header.COLUMN_NAME_WIDTH);
        // Maps "show" to "show"
        sHeadersProjectionMap.put(DBdefine.Header.COLUMN_NAME_SHOW, DBdefine.Header.COLUMN_NAME_SHOW);

        sDevicesProjectionMap = new LinkedHashMap<String, String>();
    //  sDevicesProjectionMap.put(DBdefine.Devices._ID, DBdefine.Devices._ID);
        
        sColumnsProjectionMap = new LinkedHashMap<String, String>();
        sColumnsProjectionMap.put(DBdefine.Column._ID, DBdefine.Column._ID);
        sColumnsProjectionMap.put(DBdefine.Column.COLUMN_NAME_NAME, DBdefine.Column.COLUMN_NAME_NAME);
        sColumnsProjectionMap.put(DBdefine.Column.COLUMN_NAME_TYPE, DBdefine.Column.COLUMN_NAME_TYPE);
        sColumnsProjectionMap.put(DBdefine.Column.COLUMN_NAME_TEXT, DBdefine.Column.COLUMN_NAME_TEXT);
        sColumnsProjectionMap.put(DBdefine.Column.COLUMN_NAME_VALUE, DBdefine.Column.COLUMN_NAME_VALUE);
        sColumnsProjectionMap.put(DBdefine.Column.COLUMN_NAME_SHOW, DBdefine.Column.COLUMN_NAME_SHOW);
    }
	...
	public void onCreate(SQLiteDatabase db) {
		...
		ParseXmlResource PXR = new ParseXmlResource(xmlParser);
		try {
			...
			for (iColumn = 0; iColumn < numColumn; iColumn++) {
				...
				if (xHeader.Type.equals("INTEGER")) {
					...
					try  {
						...
						if (intStr.equals("-")) number = -1;
						else number = Integer.valueOf(intStr.replace(".0", ""));
	...
	private void fillTable_Column(SQLiteDatabase db) {
		...
		int iColumn;
		int numColumn = xmlHeader.size();
		for (iColumn = 2; iColumn < numColumn; iColumn++) {	// skip the "Family" and "Part"
			DBdefine.xHeaderItem xHeader = xmlHeader.get(iColumn);
			ContentValues values = new ContentValues();
			...
			Cursor cursor = db.query(true, DBdefine.Devices.TABLE_NAME, projection, null, null, null, null, orderBy, null);
			assert(cursor != null);
			cursor.moveToFirst();
			do {
				...
				if (xHeader.Type.equals(String.valueOf("STRING"))) {	// check the value of DBdefine.Header.COLUMN_NAME_TYPE
					values.put(DBdefine.Column.COLUMN_NAME_TEXT, cursor.getString(0));
					...
					values.put(DBdefine.Column.COLUMN_NAME_TEXT, cursor.getString(0));
					Log.e(TAG, cursor.getString(0));
					values.put(DBdefine.Column.COLUMN_NAME_VALUE, cursor.getInt(0));
				}
				values.put(DBdefine.Column.COLUMN_NAME_SHOW, true);		// default value is to show this.
	...
	public void initDBM() {
		...
		if (RESET_DATABASE)
			...
			try {	// To test if the all the table are exist.
				db.rawQuery("SELECT COUNT(*) FROM " + DBdefine.Header.TABLE_NAME, null);
				db.rawQuery("SELECT COUNT(*) FROM " + DBdefine.Column.TABLE_NAME, null);
				db.rawQuery("SELECT COUNT(*) FROM " + DBdefine.Devices.TABLE_NAME, null);
			} catch (Exception exp)
			{
	...
	public void readHeaderList(ArrayList<DBdefine.HeaderItem> HeaderList, boolean All) {
		SQLiteDatabase db = this.getReadableDatabase();	
		...
		Cursor cursor = null;
		if (All)
			cursor = qbHeader.query(db, projection, null, null, null, null, DBdefine.Header.DEFAULT_SORT_ORDER);
		else {
			String selection = DBdefine.Header.COLUMN_NAME_SHOW + " <> ?";
			String[] selectionArgs = { "0" };	// Collect all non visible columns
			cursor = qbHeader.query(db, projection, selection, selectionArgs, null, null, DBdefine.Header.DEFAULT_SORT_ORDER);
		}
		assert(cursor != null);
    ...
    private void buildDevicesProjectionMap(ArrayList<DBdefine.HeaderItem> HeaderList) {
		...
		for (iHeader = 0; iHeader < nHeader; iHeader++) {
			...
			sDevicesProjectionMap.put("`"+headerItem.Name+"`", "`"+headerItem.Name+"`");
		}
    }

	private void readDeviceList(String sFamily, ArrayList<DBdefine.HeaderItem> HeaderList, ArrayList<DBdefine.DeviceRow> devList) {
		SQLiteDatabase db = this.getReadableDatabase();	
		...
		String[] projection = { DBdefine.Column.COLUMN_NAME_NAME, DBdefine.Column.COLUMN_NAME_TYPE, DBdefine.Column.COLUMN_NAME_TEXT, DBdefine.Column.COLUMN_NAME_VALUE };
		String selection = DBdefine.Column.COLUMN_NAME_SHOW + " = ?";
		String[] selectionArgs = { "0" };
		// Look for all invisible values.
		Cursor cursor = null;
		cursor = qbColumn.query(db, projection, selection, selectionArgs, null, null, DBdefine.Column.DEFAULT_SORT_ORDER);
		assert(cursor != null);
		...
		// Get the rows from table of Devices
	...
	public void getFamilies(ArrayList<String> familyList, ArrayList<Integer> cntList) {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = null;
		String sql = "SELECT SubFamily, COUNT(*) FROM " + DBdefine.Devices.TABLE_NAME + " GROUP BY SubFamily";
...
/*	public ArrayList<DBdefine.ColumnValue> getColumnValues( ArrayList<DBdefine.HeaderItem> HeaderList, ArrayList<DBdefine.DeviceRow> devList, String selColumn) {
		ArrayList<String> valueList = new ArrayList<String>();
		int col_num = 0; 
		// get col_num
		for(int iCol = 0; iCol < HeaderList.size();iCol++){
			if(HeaderList.get(iCol).Name.equals(selColumn))
				col_num = iCol;
		}
		for(DBdefine.DeviceRow Device:devList){
			if(!valueList.contains(Device.Param[col_num]))
				valueList.add("'"+Device.Param[col_num]+"'");
		}
		Log.d(TAG, valueList.toString());
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = null;
		...
		String[] selectionArgs = { selColumn };
	...
	public ArrayList<DBdefine.ColumnValue> getColumnValues(String strFamily, String selColumn) {
		SQLiteDatabase db = this.getReadableDatabase();
		SQLiteQueryBuilder qbColumn = new SQLiteQueryBuilder();
		qbColumn.setTables(DBdefine.Column.TABLE_NAME);
		qbColumn.setProjectionMap(sColumnsProjectionMap);
		String[] projection = { DBdefine.Column.COLUMN_NAME_NAME, DBdefine.Column.COLUMN_NAME_TYPE, DBdefine.Column.COLUMN_NAME_TEXT, DBdefine.Column.COLUMN_NAME_VALUE };
		String selection = DBdefine.Column.COLUMN_NAME_SHOW + " = ? AND "+DBdefine.Column.COLUMN_NAME_NAME + " != ? ";
		...
		if (strFamily.equals(String.valueOf("*")) == false) {
			...
			strSQL.append(" FROM " + DBdefine.Devices.TABLE_NAME + " WHERE SubFamily = ? ");
			SeleArgs.add(strFamily);
			if (cursor.getCount() != 0) {
				cursor.moveToFirst();
				do {
					if (strSQL.length() != 0) strSQL.append(" AND ");
			...
			if (cursor.getCount() != 0) {
				cursor.moveToFirst();
				do {
					if (strSQL.length() != 0 && !flag) strSQL.append(" AND ");
					strSQL.append("`"+cursor.getString(0) + "` <> ?");
					filterType = cursor.getString(1);
					if (filterType.equals(String.valueOf("INTEGER")))
						SeleArgs.add(cursor.getString(3));
	...
	private void setColumnValues(ArrayList<ColumnValue> cvArray, String selColumn, boolean showState) {
		SQLiteDatabase db = this.getReadableDatabase();
		String strType = "SELECT DISTINCT " + DBdefine.Column.COLUMN_NAME_TYPE + " FROM " +  DBdefine.Column.TABLE_NAME
						+ " WHERE " + DBdefine.Column.COLUMN_NAME_NAME + " = ?";
		
		String[] args = { selColumn };
		Cursor cursor = db.rawQuery(strType, args);
		assert(cursor != null);
		cursor.moveToFirst();
		boolean columnIsValue = (cursor.getString(0).equals(String.valueOf("INTEGER")));
		...
		if (strParam.length() != 0) {
			...
			db.update(DBdefine.Column.TABLE_NAME, updateValues, strWhere.toString(), whereArgs);
		}
		db.close();
	}
	...
	public void updateColumnVisibility(ArrayList<HeaderItem> hvList) {
		...
		if (whereStr.length() != 0) {
			...
			db.update(DBdefine.Header.TABLE_NAME, updateValues, whereStr.toString(), args);
		}
		db.close();
	}
}
