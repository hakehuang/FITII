...
public final class CameraManager {
	...
	public Rect getFramingRect() {
		 ...
		 * (framingRect == null) { if (camera == null) { return null; }
		 * 
		 * int width = screenResolution.x * 7 / 10; if (width < MIN_FRAME_WIDTH)
		 * { width = MIN_FRAME_WIDTH; } else if (width > MAX_FRAME_WIDTH) {
		 * width = MAX_FRAME_WIDTH; } int height = screenResolution.y * 7 / 10;
		 * if (height < MIN_FRAME_HEIGHT) { height = MIN_FRAME_HEIGHT; } else if
		 * (height > MAX_FRAME_HEIGHT) { height = MAX_FRAME_HEIGHT; }
		 * 
		 * int leftOffset = (screenResolution.x - width) / 2; int topOffset =
		 * (screenResolution.y - height) / 4; framingRect = new Rect(leftOffset,
